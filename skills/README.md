...
| [`ig-translate/`](ig-translate/SKILL.md) | Multi-language support in **template-owner scope**: the de-default language policy and its guard, the language-neutrality of the header/footer/CSS overrides, the vendored German UI strings. The rendering matrix itself lives in the catalog's `fhir-ig-translation`; this skill only points at it. |
