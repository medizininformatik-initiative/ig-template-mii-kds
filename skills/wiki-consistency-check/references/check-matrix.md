...
| Language (content) | `sushi-config.yaml` (`i18n-default-lang`), `input/fsh` | Namenskonventionen → Sprache | Resource `description`/`name`/`title` may be German, and then a translation extension is required on them? Which language the guide leads in (`i18n-default-lang`) is the module's own decision — this template and the KDS Dokument module lead in German, `kerndatensatz-basis` leads in English — and § Sprache states no rule either way, so report the setting and do not call another choice a deviation. | Module |
