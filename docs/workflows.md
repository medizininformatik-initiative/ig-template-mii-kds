...
| --- | --- | --- | --- | --- | --- |
| `ig-preview.yml` | push to any branch except `main`/`gh-pages`/`fsh-generated`; `workflow_dispatch` | Builds the **preview IG** (SUSHI + IG Publisher), deploys a preview and regenerates the root index | `gh-pages/branches/<branch>/` + the regenerated `gh-pages/index.html` + PR comment with the URL | `ENABLE_PREVIEW` (ON) | no |
| `release-demo.yml` | `release: published`; `workflow_dispatch` (inputs `tag`, `update_landing_page`) | Rebuilds the demo IG **from the released tag** with the same pinned toolchain, verifies the render carries that version, publishes it, and regenerates the Pages root index | `gh-pages/demo/<tag>/` + the regenerated `gh-pages/index.html` | `ENABLE_RELEASE_DEMO` (ON) | reacts only to a release a human published |
...
- **The published demo tracks the latest release automatically.** The Pages
  root index lists one rendering as the latest release; since
  ...
  `demo/<tag>/`, and the root index is regenerated to list it in the same
  `gh-pages` commit. See
...
- **The `tooling-tests` job** runs the repository's script tests
  ...
  `toolchain-pins.test.mjs` was added with `release-demo.yml` — it fails when
  the two build workflows stop pinning the same toolchain.
  `gen-pages-index.test.mjs` pins the root-index generator's contract (§ 4).
...
The [Pages root index](https://medizininformatik-initiative.github.io/ig-template-mii-kds/)
lists one rendering of this repository's preview IG as *the latest release*.
**It tracks the latest release automatically** — `release-demo.yml`
...
| | |
| --- | --- |
| **Built from the tag** | The job checks out the released tag into `release-src/` and builds only that. The workspace root stays on the workflow's own ref so the publishing tooling (`scripts/gen-pages-index.mjs`) is current even when re-rendering an old tag. |
...
| **The links follow** | `gh-pages/index.html` is regenerated in the same commit as the demo it points at, from what is actually deployed in the tree. Path, rendered content and link therefore move together or not at all, and the index can never link something that does not exist. |
...
`index.html` on `gh-pages` is **generated** by `scripts/gen-pages-index.mjs` —
never edited by hand (the page says so in an HTML comment). It replaced the
hand-authored landing page and its conservative link rewriter (both retired
2026-08-28): a plain,
autoindex-style, self-contained page listing exactly two things —
`branches/dev/` (the development preview) and the highest-SemVer `demo/<tag>/`
(the latest release demo, i.e. the released state of `main`; `main`
deliberately has no branch preview). The generator links only what exists in
the `gh-pages` tree at generation time: a demo without a root `index.html` is
listed via its real entry points (`de/`, `en/`, `qa.html`). Both
`release-demo.yml` and `ig-preview.yml` run it before pushing, so the index
heals on every deploy and is created from scratch when missing.
`scripts/gen-pages-index.test.mjs` pins this contract.

Dry-run it against a checkout (or any fixture tree) before changing the script:

```bash
git clone --depth 1 --branch gh-pages \
  https://github.com/medizininformatik-initiative/ig-template-mii-kds /tmp/gh-pages
node scripts/gen-pages-index.mjs /tmp/gh-pages --check
```
...
deliberately writes none (a guard fails the job if one appears). Only the root
index's *latest* link moves; superseded demos stay reachable at their stable
URLs, deliberately unlisted. The job prints the accumulated `demo/` size and warns above the
