...
| Is the package published to a FHIR package registry? | **No.** Modules consume the template as a vendored folder, re-vendored from this repo's `dev` branch by the module template's `scripts/sync-ig-template.sh` — see [how a module consumes this template](workflows.md#how-a-module-consumes-this-template). Publication is the explicit decision [#6](../../../issues/6). |
