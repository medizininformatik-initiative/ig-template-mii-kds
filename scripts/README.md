...
| `validate.test.mjs` | Guards the instance-validation page (`content/validate.html` + `assets/js/validate.js`): the `POST /validate` request body against the verified validator-wrapper API, the issue table (wrapper `level` -> severity, HTML-escaped), the public-validator fallback for an unset `features.validator.url`, both language sections with the four routes and the data-protection box, the root-aware footer link, and no external assets | `security-scan.yml`, `dependency-check.yml` |
